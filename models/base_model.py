...
class BaseModel(ABC):
    ...
    def load_weights(self, weights_path):
        net = getattr(self, 'fodnet')
        if isinstance(net, torch.nn.DataParallel):
            net = net.module
        print('loading the model from %s' % weights_path)
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if isinstance(net, list):
                    for index, subnet in enumerate(net):
                        num_params = 0
                        for param in subnet.parameters():
                            num_params += param.numel()
                        if verbose:
                            print(subnet)
                        print('[Network %s_%s] Total number of parameters : %.3f M' % (name, index, num_params / 1e6))
                else:
                    num_params = 0
                    for pname, param in net.named_parameters():
                        num_params += param.numel()
                        print(pname, param.size(), param.numel() / 1e6)
                    if verbose:
                        print(net)
